    ...
    <div className="bg-white rounded-2xl border border-slate-100">
      <div className="p-5 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Select Columns to Encode</h2>
            <p className="text-sm text-slate-500">Choose which categorical columns to apply encoding</p>
          </div>
        </div>
      </div>
        ...
        {statsLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-500">
              {searchColumn ? "No columns found matching your search" : "No categorical columns found in the dataset"}
            </p>
            {!searchColumn && (
              <p className="text-xs text-slate-400 mt-2">
                Categorical columns contain text/string values that can be encoded for machine learning.
              </p>
            )}
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {stats ? (
                        <div className="flex flex-wrap gap-1">
                          <span className={`text-xs px-1.5 py-0.5 rounded ${isSelected ? "text-purple-600 bg-purple-100" : "text-slate-500 bg-slate-100"}`}>
